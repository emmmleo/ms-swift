      ...
      <el-page-header @back="$router.push('/dashboard')">
        <template #content>
          <div class="header-content">
            <span class="text-large font-600 mr-3">⚖️ 人类偏好对齐 (RLHF / DPO)</span>
            <el-tag type="warning" effect="plain" round>Reinforcement Learning from Human Feedback</el-tag>
          </div>
        </template>
      </el-page-header>
    </el-header>
            ...
            <el-tab-pane label="LoRA / Tuner" name="lora">
              <el-form :model="form" label-width="160px" label-position="left">
                <el-form-item label="使用 LoRA">
                  ...
                  <div class="form-tip">RLHF 通常也可以使用 LoRA 进行参数高效微调</div>
                </el-form-item>
...
const form = ref({
  ...
  dataset: [], // changed to array for multiple select
...
const handleLaunch = async () => {
  ...
  try {
    ...
    const fields = [
      ...
      'gradient_accumulation_steps', 'max_length', 'beta',
      'seed', 'dtype',
      'ref_model_id', 'deepspeed'
    ]
    ...
    // LoRA params (only if use_lora is true, though swift rlhf might imply lora or full based on other params. 
    // Usually rlhf supports sft_type lora/full. Let's assume lora is default or we need to pass sft_type)
